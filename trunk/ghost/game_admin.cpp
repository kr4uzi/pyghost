...
void CAdminGame :: EventPlayerJoined( CPotentialPlayer *potential, CIncomingJoinPlayer *joinPlayer, bool executeTwice )
{
	try	
	{ 
		EXECUTE_HANDLER("PlayerJoinedAdminGame", true, boost::ref(this), boost::ref(potential), boost::ref(joinPlayer)) 
	}
	catch(...) 
	{ 
		return;
	...
	EXECUTE_HANDLER("PlayerJoinedAdminGame", false, boost::ref(this), boost::ref(potential), boost::ref(joinPlayer))
}

bool CAdminGame :: EventPlayerBotCommand( CGamePlayer *player, string command, string payload, bool executeTwice )
{
	try	
	{ 
		EXECUTE_HANDLER("PlayerBotCommandAdminGame", true, boost::ref(this), boost::ref(player), command, payload) 
	}
	catch(...) 
	{ 
		return true;
	...
	CBaseGame :: EventPlayerBotCommand( player, command, payload, true );
	...
	*********************/
	...
	EXECUTE_HANDLER("PlayerBotCommandAdminGame", false, boost::ref(this), boost::ref(player), command, payload)
...
void CAdminGame :: RegisterPythonClass( )
{
	using namespace boost::python;

	class_< CAdminGame, bases<CBaseGame> >("adminGame", no_init)
		.def_readwrite("password", &CAdminGame::m_Password)

		.def_readonly("tempBans", &CAdminGame::m_TempBans)
		.def_readonly("pairedAdminCounts", &CAdminGame::m_PairedAdminCounts)
		.def_readonly("pairedAdminAdds", &CAdminGame::m_PairedAdminAdds)
		.def_readonly("pairedAdminRemoves", &CAdminGame::m_PairedAdminRemoves)
		.def_readonly("pairedBanCounts", &CAdminGame::m_PairedBanCounts)
		.def_readonly("pairedBanRemoves", &CAdminGame::m_PairedBanRemoves)

		.def("sendAdminChat", &CAdminGame::SendAdminChat)
		.def("sendWelcomeMessage", &CAdminGame::SendWelcomeMessage)
	;
}
